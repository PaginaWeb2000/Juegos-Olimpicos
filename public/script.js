...
function trackMedalDate(type, participantName) {
  ...
  // Verificar logro de Rompecabezas
  checkPuzzleAchievement(medalDates, newMedal);
}

function checkPuzzleAchievement(medalHistory, latestMedal) {
  // Solo verificar si hay al menos 3 medallas
  if (medalHistory.length < 3) return;
  
  // Obtener las últimas 3 medallas
  const lastThree = medalHistory.slice(-3);
  
  // Verificar el patrón: oro, bronce, plata (gold, bronze, silver)
  // Las medallas están ordenadas por timestamp, siendo [0] la más antigua
  ...
  if (isPuzzlePattern && !unlockedAchievements.puzzle) {
    // Marcar logro como desbloqueado
    unlockedAchievements.puzzle = true;
    localStorage.setItem('unlockedAchievements', JSON.stringify(unlockedAchievements));
    
    // Mostrar notificación
    showAchievementNotification(
      '🧩 Rompecabezas',
      `¡Has completado el patrón de medallas: Oro, Bronce, Plata!`
    );
  }
}
...
function guardarHistorialClasificacion() {
  try {
    ...
    if (clasificacionAnterior.length > 0) {
      ...
      // Verificar logro 'Cambio de Suerte' (pasar de último a primero)
      if (clasificacionAnterior.length > 0) {
        // Obtener el último de la clasificación anterior
        const ultimoAnterior = clasificacionAnterior[clasificacionAnterior.length - 1];
        
        // Verificar si ahora es el primero
        const ahoraPrimero = clasificacionActual[0];
        
        // Si el que ahora es primero era el último en la clasificación anterior
        if (ahoraPrimero.nombre === ultimoAnterior.nombre) {
          // Inicializar el objeto si no existe
          let cambiosDeSuerte = JSON.parse(localStorage.getItem('cambioDeSuerte') || '{}');
          
          // Verificar que no haya perdido posiciones intermedias
          const posicionAnterior = clasificacionAnterior.findIndex(p => p.nombre === ahoraPrimero.nombre) + 1;
          
          // Si estaba último y ahora es primero, otorgar el logro
          if (posicionAnterior === clasificacionAnterior.length) {
            // Asegurarse de no sobrescribir logros previos
...
function renderTable() {
  ...
  sorted.forEach((p, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        ${p.gold}
        <button class="icon-btn add-medal-btn" title="Añadir oro" data-type="gold" data-index="${participants.findIndex(part => part.name === p.name)}">
          <span aria-hidden="true">➕</span>
        </button>
      </td>
      <td>
        ${p.silver}
        <button class="icon-btn add-medal-btn" title="Añadir plata" data-type="silver" data-index="${participants.findIndex(part => part.name === p.name)}">
          <span aria-hidden="true">➕</span>
        </button>
      </td>
      <td>
        ${p.bronze}
        <button class="icon-btn add-medal-btn" title="Añadir bronce" data-type="bronze" data-index="${participants.findIndex(part => part.name === p.name)}">
          <span aria-hidden="true">➕</span>
        </button>
      </td>
    ...
    tr.querySelectorAll('.add-medal-btn').forEach(btn => {
      btn.onclick = (e) => {
        e.stopPropagation();
...
// Inicializar logros desbloqueados si no existen
if (!localStorage.getItem('unlockedAchievements')) {
  localStorage.setItem('unlockedAchievements', JSON.stringify({}));
}

function showAchievementNotification(titulo, mensaje) {
  // Crear el elemento de notificación
  const notificacion = document.createElement('div');
  notificacion.className = 'achievement-notification';
