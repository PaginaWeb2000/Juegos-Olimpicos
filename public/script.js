...
function trackMedalDate(type, participantName) {
  ...
  // Verificar si se ha completado el logro "Rompecabezas" (Oro, Bronce, Plata)
  ...
  try {
    ...
    if (clasificacionAnterior.length > 0) {
      ...
      if (clasificacionAnterior.length > 0) {
        ...
        if (ahoraPrimero && ahoraPrimero.nombre === ultimoAnterior?.nombre) {
          ...
          // Si estaba último y ahora es primero, otorgar el logro "De Cero a Cien"
          if (posicionAnterior === clasificacionAnterior.length) {
            ...
            if (!cambiosDeSuerte[ahoraPrimero.nombre]) {
              ...
              // Mostrar notificación visual (si existe la función showNotification)
        ...
        // Verificación independiente para el logro "Caída Libre" (primero a último)
        const antesPrimero = clasificacionAnterior[0]?.nombre;
        if (antesPrimero && clasificacionActual.length > 0) {
          const ahoraUltimo = clasificacionActual[clasificacionActual.length - 1]?.nombre;
          
          // Si el que antes era primero ahora es último
          if (antesPrimero === ahoraUltimo) {
            // Obtener o inicializar el registro de caídas libres
            let caidasLibres = JSON.parse(localStorage.getItem('caidasLibres') || '{}');
            
            // Si no tiene el logro aún, se lo otorgamos
            if (!caidasLibres[antesPrimero]) {
              caidasLibres[antesPrimero] = true;
              localStorage.setItem('caidasLibres', JSON.stringify(caidasLibres));
              
              // Mostrar notificación de logro desbloqueado
              console.log(`¡${antesPrimero} ha conseguido el logro 'Caída Libre'!`);
              
              // Mostrar notificación visual (si existe la función showNotification)
              if (typeof showNotification === 'function') {
                showNotification(`¡${antesPrimero} ha conseguido el logro 'Caída Libre'!`);
              }
            }
          }
        }
