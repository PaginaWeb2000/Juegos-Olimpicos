...
// ========== INICIALIZACIÓN ==========
async function init() {
  // 1. Intenta cargar desde el backend (Netlify)
  try {
    const response = await fetch('/.netlify/functions/cargar');
    if (response.ok) {
      const result = await response.json();
      if (result.data && Array.isArray(result.data) && result.data.length > 0) {
        participants = result.data;
        saveParticipants(); // sincroniza localStorage
        renderTable();
        return;
      }
    }
  } catch (e) {
    // Si falla, sigue con localStorage
  }
  // 2. Si no hay datos en backend, usa localStorage o los default
